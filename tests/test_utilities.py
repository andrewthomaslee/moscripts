...
def test_create_human_readable_timestamp():
    assert create_human_readable_timestamp() == create_human_readable_timestamp(datetime.now(timezone.utc))
...
def test_nix_run_prefix_subprocess_nixpkgs_download():
    ...
    result = subprocess.run([*nix_run_prefix("mpv"), "--version"], capture_output=True, text=True)
